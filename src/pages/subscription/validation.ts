...
const planStep = defineValidationSchema<FormShape>(({ model }) => {
  ...
  validate(model.$.billing, [required({ message: 'Выберите периодичность оплаты' })]);
});

const contactsStep = defineValidationSchema<FormShape>(({ model }) => {
  validate(model.$.contactName, [required({ message: 'Укажите имя' }), minLength(2)]);
...
const STEP_SCHEMAS: readonly ValidationSchema<FormShape>[] = [planStep, contactsStep];
...
const fullSchema = defineValidationSchema<FormShape>(() => apply(planStep, contactsStep));
 ...
 */
export function makeValidationConfig(model: FormModel<FormShape>, form: FormProxy<FormShape>) {
  return {
